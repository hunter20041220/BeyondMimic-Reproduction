#!/usr/bin/env python3
"""Run a live Isaac unguided diffusion+VAE closed-loop rollout."""
...
def main() -> int:
    ...
    try:
        ...
        summary["validation_mode"] = "unguided_diffusion_closed_loop"
        output_dir = Path(args.output_dir)
        output_dir.mkdir(parents=True, exist_ok=True)
        (output_dir / "eval_diffusion_closed_loop_summary.json").write_text(
        ...
        error = {
            "status": "error",
            "entrypoint": "eval_diffusion_closed_loop",
            "error_type": type(exc).__name__,
            "error": str(exc),
            "traceback": traceback.format_exc(),
        }
        (output_dir / "eval_diffusion_closed_loop_error.json").write_text(
