#!/usr/bin/env python3
"""Collect a live Isaac VAE rollout for Stage-3 data.
...
def _add_common_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--ou-noise-seconds", type=float)
...
def main() -> int:
    ...
    try:
        ...
        (output_dir / "collect_vae_rollout_summary.json").write_text(
        ...
        error = {
            "status": "error",
            "entrypoint": "collect_vae_rollout",
            "error_type": type(exc).__name__,
            "error": str(exc),
            "traceback": traceback.format_exc(),
        }
        (output_dir / "collect_vae_rollout_error.json").write_text(
